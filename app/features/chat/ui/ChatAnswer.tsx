...
const ChatAnswer = memo(function ChatAnswer({
  ...
  const answerText = [...answer.options, ...answer.answer].filter((item) => item.length).join(", ");

  if (!answerText.length && !answer.audio) return null;

  return (
    <div className="flex justify-end relative mb-5 gap-3">
      {answer.warning && <WarningIcon className="w-5 h-5 fill-red-900 self-center shrink-0" />}
      <div
